...
class AgentRunnerTest {
    ...
    fun `FINAL_ANSWER stops loop immediately`() = runTest {
        ...
        assertTrue(finalIndex >= 0, "FINAL_ANSWER step not found")
        val stepsAfter = steps.drop(finalIndex + 1)
        assertTrue(stepsAfter.isEmpty(), "Expected no steps after FINAL_ANSWER, got: $stepsAfter")
    }
    ...
    fun `loop stops after at most 6 iterations`() = runTest {
        ...
        assertTrue(actionSteps.size <= 6, "Expected <= 6 ACTION steps, got ${actionSteps.size}")
    }
...
// ---------------------------------------------------------------------------
// Shared test infrastructure (used by AgentRunnerTest and other tests)
// ---------------------------------------------------------------------------
